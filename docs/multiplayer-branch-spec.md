...
- **`FixBackupRamTimeIndex(dll)`** — corrects one byte of the backup-RAM block the module injects, without which the service menu's **GAME ASSIGNMENTS** page hard-freezes the whole application. This is a bug in SEGA's module, not in YAMP; it only became reachable once `InstallSystemSwitches` made the TEST switch work, which is why Lost Judgment itself never hit it. The chain is worth stating in full because it is the clearest example of the emulator's most dangerous divergence from hardware:
  ...
  - **The byte has a second consumer, and fixing it alone is not enough.** HLE hook 16 (`GAME_INT+0x4`, handler `+0x52CA0`) is a two-line handler that copies backup `+0x3351` **raw** into `time` (RAM `0x500090`) on every game interrupt, then runs the original instruction. Those are different units: `time` is the round length in **seconds** — the ROM's own writers put a literal `30` there (`0x6CEC`) and its debug setter clamps to `0..0x63` — reached from the index via `time = time_vars[+0x3351]` in `main` (`0x7200`). Because the hook fires continuously it always beats `main`'s one-shot conversion, which is *why* the module could ship a seconds value in an index field and never notice. With the injector corrected, that same hook forces `time = 2`. YAMP therefore ships hook 16 **disabled by default** (`HleHooks::DEFAULT_DISABLE_MASK`), which hands the round length back to the ROM and makes the operator's GAME ASSIGNMENTS setting actually reach it. The hook has no other effect, so disabling it costs nothing.
  - The two halves are one fix and are documented as such in both tooltips and in the hook's own description, because **fix on + hook on is the one combination that breaks gameplay** (≈2-second matches) and it is otherwise invisible. Both off is the module's stock behaviour: a working timer and a page that freezes the emulator. The hook-list preset formerly labelled "Disable none" is now **"Restore defaults"** and sets `DEFAULT_DISABLE_MASK` rather than `0`, so the preset cannot silently re-arm the bad combination; the hook's own checkbox still re-enables it in one click.
  - `DEFAULT_DISABLE_MASK` is also what the netplay override restores instead of `{0,0}`. That does not weaken the determinism argument in §14.8 — it is a compiled-in constant, not a setting, so peers on the same build agree on it without exchanging anything, which is exactly the property the all-hooks-restored rule buys. Restoring hook 16 there would only make both peers agree on a two-second round.
...
`HleHooks::Update()` is called once per frame; it restores or re-applies each hook to match the setting, working live off the DLL's own save area. The setting does not default to "nothing disabled": `DEFAULT_DISABLE_MASK` ships hook 16 (`GAME_INT+0x4`) off, because it contradicts the backup-RAM TIME correction — see §9.3.
...
| | `DisabledHleHooksLo` / `...Hi` | hex u64 | **`HleHooks::DEFAULT_DISABLE_MASK`** (bit 16 set) | Bit i = hook i. **Core bits stripped on save and load.** Default is no longer 0 — see §9.3; an ini from an older build carries an explicit `0` and keeps hook 16 enabled until the box is re-ticked |
