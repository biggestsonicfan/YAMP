...
namespace Cabinet
{
	...
	void PollSystemSwitches(bool suppressed, void (*set)(bool test, bool service), bool forceTest)
	{
		bool test = forceTest;
		bool service = false;
		if (!suppressed)
	...
	void RoutePads(pxd::csl_pad pads[2], bool netplayMatch, int32_t netplayLocalPad)
	{
		...
		{
			// The other slot is filled from the P2 bindings purely to keep it well-formed for
			// this frame: NetSession::Step() overwrites BOTH pads from the transmitted inputs
			// before module_main sees them, so nothing local survives into the simulation (see
			// PadCodec.h).
