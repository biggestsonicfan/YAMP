...
// pxd shader constant-buffer layouts. These are the same in every pxd generation YAMP hosts, so
// this header is shared: both source/pxd/LJ/gs.h (the Lost-Judgment-era platform layer) and
// source/vf5fs/Y6/gs.h (the Yakuza 6-era one) include it.
//
// About this directory: source/pxd holds pxd ENGINE code, as opposed to YAMP's own host/app code in
// source root. Today that is the generation-neutral headers plus the LJ-era platform layer (sl
// 0xF000 / gs 0x388A00 / ct 0x30, DX12 host cdevice) shared by the m2ftg hosts and the LJ VF5FS
// host. If a second generation's platform layer is ever promoted out of a host folder, give each
// one a subdirectory (source/pxd/LJ, source/pxd/Y6) and keep the neutral headers here.
