...
namespace pxd
{
        ...
        namespace sl {
            ...
            uint32_t* p_sync_archive_condvar;

            uint32_t* sync_archive_condvar()
            {
                return p_sync_archive_condvar != nullptr ? p_sync_archive_condvar
                                                         : &sm_context->sync_archive_condvar;
            }
            ...
            int initialize(uint32_t context_size)
            {
                ...
                // context header: it requires size_of_struct (+8) to match its generation,
                // silently leaving sm_context unset otherwise. Hence the parameter.
                sm_context->size_of_struct = context_size;
