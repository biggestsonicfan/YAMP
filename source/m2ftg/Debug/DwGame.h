...
// The per-game "dw" descriptor layer shared by the two halves of the old DebugWindows.cpp:
// the DwGame table (module DLL names, boot-state / context / config RVAs, the memory-map
// dispatch offsets) and the primitives that read the live machine through it. Split out
// 2026-08-09 so Determinism.cpp (the netplay API) and DebugWindows.cpp (the ImGui panes)
// stop sharing one 1,700-line file; everything here is header-inline because both TUs use
// it and none of it holds cross-TU state.
...
namespace dwdbg
{
	...
	inline const std::unordered_map<uintptr_t, const char*> RVA_NOTES =
	{
	...
	inline bool BoardBooted(const DwGame* game, const uint8_t* base) noexcept
	{
	...
	inline unsigned long long InvokeDwAction(void* fn) noexcept
	{
		__try
		{
	...
	inline bool ReadEmulated32(uint8_t* base, uint32_t address, uint32_t& out) noexcept
	{
	...
	inline bool WriteEmulated32(uint8_t* base, uint32_t address, uint32_t value) noexcept
	{
		const uint32_t index = address < 0x3000000 ? (address >> 20) : ((address >> 28) + 0x30);
		if (index >= MEMMAP_RECORD_COUNT)
		{
	...
	inline void SymbolizeI960(uint8_t* base, uint32_t address, char* buffer, size_t bufferSize)
	{
