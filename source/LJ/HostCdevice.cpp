...
void DumpDredNow();
void D3D12LogLine(const char* s);
	...
	namespace StF
	{
		...
		namespace
		{
				...
				uint32_t tag, int /*zero*/, const wchar_t* name, void** out)
			{
				...
				{
					char buf[128];
					_snprintf_s(buf, _TRUNCATE, "[StF cdevice] CreateCommittedResource failed hr=0x%08X\n", hr);
					OutputDebugStringA(buf);
			...
			void SeedDeferredNodePool()
			{
				...
				{
					char dbg[128];
					_snprintf_s(dbg, _TRUNCATE, "[StF cdevice] deferred pool ctor: base=%p count=%u\n",
						reinterpret_cast<void*>(base), count);
				...
				if (!base || count == 0 || count > 256)
				{
					...
					CdevU32(0xB8) = count;
					OutputDebugStringA("[StF cdevice] deferred pool: self-provisioned nodes @ cdevice+0xC8 x0x40\n");
				}
			...
			static void SeqLog(const char* s) { if (StfRenderActive() && g_seqLog < 140) { g_seqLog++; D3D12LogLine(s); } }
				...
				UINT vpi, UINT inst, UINT svl, UINT sil)
			{
				...
				{ char b[96]; _snprintf_s(b,_TRUNCATE,"[draw] instanced=%llu indexed=%llu vertsTotal=%llu\n",
					(unsigned long long)g_drawInstCount,(unsigned long long)g_drawIdxCount,(unsigned long long)g_drawVertsTotal); D3D12LogLine(b); }
				...
				UINT ipi, UINT inst, UINT sil, INT bvl, UINT sivl)
			{
				...
				if (IsListClosed(self))
				{
					...
					{
						char b[128];
						_snprintf_s(b, _TRUNCATE, "[closed-draw] DrawIndexed on CLOSED list %p (dropped by runtime)\n",
			...
			void RegisterWatchResource(ID3D12Resource* r)
			{
				...
				char b[96]; _snprintf_s(b, _TRUNCATE, "[watch] registered backbuffer #%d = %p\n", g_watchCount - 1, static_cast<void*>(r));
				D3D12LogLine(b);
			}
				...
				if (bars)
				{
					for (UINT i = 0; i < n; ++i)
					{
						...
						if (IsWatched(r))
						{
							...
							{
								char b[160];
								_snprintf_s(b, _TRUNCATE, "[watch-bar] res=%p %s before=0x%X after=0x%X\n",
						...
						if (bars[i].Transition.StateAfter == D3D12_RESOURCE_STATE_RENDER_TARGET)
						{
							...
							if (!seen && g_rtSeenCount < 64)
							{
								...
								char b[160];
								_snprintf_s(b, _TRUNCATE, "[rt] #%d res=%p W=%llu H=%u fmt=%d flags=0x%X\n",
				...
				D3D12_CPU_DESCRIPTOR_HANDLE h, const FLOAT c[4], UINT nr, const D3D12_RECT* rects)
			{
				...
				if (s_n < 24 && c) { ++s_n;
					char b[160];
					_snprintf_s(b, _TRUNCATE, "[clear] RTV=0x%llX color=(%.3f, %.3f, %.3f, %.3f) inStf=%d rects=%u list=%p\n",
			...
			void MarkStfRenderList(ID3D12GraphicsCommandList* l) // "this list is StF's" (from the PSO hook)
			{
				...
				static int m = 0; if (m < 8) { char b[96]; _snprintf_s(b, _TRUNCATE, "[mark] StF list %p type=%d (flagged=%d)\n", static_cast<void*>(l), l->GetType(), g_stfFlaggedCount); D3D12LogLine(b); } m++;
			}
			...
			bool StfExecDisabled() { return g_execDisabled; }
				...
				void SubmitStfFrameList()
				{
					...
					for (int i = 0; i < g_stfFlaggedCount && nn < 64; ++i)
					{
						...
						if (FAILED(hrc))
						{
							static int e = 0; if (e++ < 8) { char b[112]; _snprintf_s(b, _TRUNCATE, "[pathb] Close failed 0x%08X list=%p (skip)\n", static_cast<unsigned>(hrc), static_cast<void*>(l)); D3D12LogLine(b); }
							continue;
						}
						...
						if (cbf < 20 && g_cbDumpRes && g_cbDumpCount) // log the CPU addr a while so there's time to set the x64dbg bp
						{
							...
							if (SUCCEEDED(g_cbDumpRes->Map(0, &rr, &p)) && p)
							{
								for (int i = 0; i < g_cbDumpCount; ++i)
								{
									...
									char b[288]; _snprintf_s(b, _TRUNCATE, "[cbv-submit] off=%llu [%.3f %.3f %.3f %.3f | %.3f %.3f %.3f %.3f | %.3f %.3f %.3f %.3f | %.3f %.3f %.3f %.3f]\n",
					...
					static int f = 0; if (f < 8) { char b[128]; _snprintf_s(b, _TRUNCATE, "[pathb] submitted %d StF list(s)%s\n", nn, timedOut ? " (FLUSH TIMEOUT - GPU HANG)" : ""); D3D12LogLine(b); } f++;

					const HRESULT rr = s_device->GetDeviceRemovedReason();
					if (timedOut || FAILED(rr))
					{
						char b[128]; _snprintf_s(b, _TRUNCATE, "[pathb] device removed/hang (0x%08X) disabling + DRED\n", static_cast<unsigned>(rr)); D3D12LogLine(b); DumpDredNow(); g_execDisabled = true; return;
					}
			...
			void AdvanceFrameStamp()
			{
				...
				{
					char b[128];
					_snprintf_s(b, _TRUNCATE, "[stamp] frame=%d stamp=%llu recycled=%u uploadRefills=%u\n",
			...
			{
				{
					...
					if ((s_fr < 40 || s_fr % 20 == 0) && s_fr < 2000) { char b[128]; _snprintf_s(b, _TRUNCATE,
						"[frame %d] draws this frame: indexed=%llu instanced=%llu, flaggedLists=%d\n",
						s_fr, (unsigned long long)di, (unsigned long long)dn, g_stfFlaggedCount); D3D12LogLine(b); }
					// The exact StF draw list + whether StF EVER closes/resets/executes it (the missing submit).
					if (s_fr < 40 || s_fr % 20 == 0) { char b[160]; _snprintf_s(b, _TRUNCATE,
						...
						static_cast<void*>(&g_lastStfCmdList)); D3D12LogLine(b); }
					...
					// [vp-tally] per-frame deltas: does the 3D pass (vp height 768) record draws?
					{
						...
						if (s_fr < 40 || s_fr % 20 == 0) { char b[160]; _snprintf_s(b, _TRUNCATE,
							...
							(unsigned long long)d384, (unsigned long long)dOther); D3D12LogLine(b); }
					}
					// GBV id=547 tally: draws landing on a CLOSED list are silently dropped by the runtime.
					if (s_fr < 40 || s_fr % 20 == 0) { char b[128]; _snprintf_s(b, _TRUNCATE,
				...
				if (n)
				{
					...
					bool timedOut = false;
					if (g_execFence) { g_yampQueue->Signal(g_execFence, ++g_execFv); if (g_execFence->GetCompletedValue() < g_execFv) { g_execFence->SetEventOnCompletion(g_execFv, g_execEv); timedOut = (WaitForSingleObject(g_execEv, 4000) == WAIT_TIMEOUT); } }
					static int f = 0; if (f < 8) { char b[96]; _snprintf_s(b, _TRUNCATE, "[exec-fix] frame executed %d StF list(s) in ONE submit%s\n", n, timedOut ? " (FLUSH TIMED OUT - GPU HANG)" : ""); D3D12LogLine(b); } f++;
					if (timedOut) { D3D12LogLine("[exec-fix] GPU did not complete in 4s -> hang on StF draws; disabling + dumping DRED\n"); DumpDredNow(); g_execDisabled = true; }
					const HRESULT rr = s_device->GetDeviceRemovedReason();
					if (FAILED(rr))
					{
						char b[128]; _snprintf_s(b, _TRUNCATE, "[exec-fix] DEVICE REMOVED 0x%08X after submit — disabling StF execution\n", static_cast<unsigned>(rr));
						OutputDebugStringA(b); D3D12LogLine(b); DumpDredNow();
						g_execDisabled = true;
					}
			...
			static HRESULT STDMETHODCALLTYPE HookedClose(ID3D12GraphicsCommandList* self)
			{
				...
				{
					static int c = 0; if (c < 16) { char b[96]; _snprintf_s(b, _TRUNCATE, "[close] StF-flagged list %p closed by StF\n", static_cast<void*>(self)); D3D12LogLine(b); } c++;
				}
				return hr;
			}
			static HRESULT STDMETHODCALLTYPE HookedReset(ID3D12GraphicsCommandList* self, ID3D12CommandAllocator* a, ID3D12PipelineState* p)
			{
				...
				static int c = 0; if (c < 8 || c % 600 == 0) { char b[96]; _snprintf_s(b, _TRUNCATE, "[cmdlist] Reset self=%p alloc=%p #%d\n", static_cast<void*>(self), static_cast<void*>(a), c); OutputDebugStringA(b); } c++;
			...
			static HRESULT STDMETHODCALLTYPE HookedCreateGfxPSO(ID3D12Device* self, const D3D12_GRAPHICS_PIPELINE_STATE_DESC* desc, REFIID riid, void** ppv)
			{
				...
				{
					static int s = 0;
					if (s < 24) { s++; char b[192]; _snprintf_s(b, _TRUNCATE,
						...
						desc->NumRenderTargets, desc->RTVFormats[0], desc->DSVFormat); D3D12LogLine(b); }
				}
			...
			static void STDMETHODCALLTYPE HookedCreateCBV(ID3D12Device* self, const D3D12_CONSTANT_BUFFER_VIEW_DESC* d, D3D12_CPU_DESCRIPTOR_HANDLE h)
			{
				if (StfRenderActive() && d && g_cbvLog < 12)
				{
					...
					char b[320];
					_snprintf_s(b, _TRUNCATE, "[cbv] va=0x%llX size=%u res=%p off=%llu got=%d\n     [%.3f %.3f %.3f %.3f | %.3f %.3f %.3f %.3f | %.3f %.3f %.3f %.3f | %.3f %.3f %.3f %.3f]\n",
			...
			static void STDMETHODCALLTYPE HookedRSSetScissorRects(ID3D12GraphicsCommandList* self, UINT n, const D3D12_RECT* rects)
			{
				if (StfRenderActive() && rects && n)
				{
					...
					{
						char b[192];
						_snprintf_s(b, _TRUNCATE, "[scissor] n=%u [0] l=%ld t=%ld r=%ld b=%ld (w=%ld h=%ld) list=%p\n",
			...
			static void STDMETHODCALLTYPE HookedIASetVertexBuffers(ID3D12GraphicsCommandList* self, UINT start, UINT num, const D3D12_VERTEX_BUFFER_VIEW* v)
			{
				...
				// can save it — the missing piece would be the VB upload/copy, not transforms.
				{
					...
					if (StfRenderActive() && s_vbDump < 10 && v != nullptr && num != 0 && v[0].BufferLocation != 0)
					{
						...
						char b2[240];
						_snprintf_s(b2, _TRUNCATE, "[vbdump] va=0x%llX res=%p off=%llu map=%d nz256=%u [%.2f %.2f %.2f %.2f %.2f %.2f %.2f %.2f]\n",
							static_cast<unsigned long long>(v[0].BufferLocation), static_cast<void*>(res), off, got, nz,
							f[0], f[1], f[2], f[3], f[4], f[5], f[6], f[7]);
						D3D12LogLine(b2); s_vbDump++;
					}
				}
				...
				if (fromStf || StfRenderActive())
				{
					...
					if (g_copyLog < 16)
					{
						...
						// Check the bits before trusting the float reading of lane 3.
						{
							...
							char bh[224];
							_snprintf_s(bh, _TRUNCATE, "[copy-hex] %08X %08X %08X %08X | %08X %08X %08X %08X\n",
								uu[0], uu[1], uu[2], uu[3], uu[4], uu[5], uu[6], uu[7]);
							D3D12LogLine(bh);
						}
						char b[448];
						_snprintf_s(b, _TRUNCATE, "[copy] dstVA=0x%llX bytes=%llu srcMap=%d nz4k=%u nan/1024=%u first=%d stride=%d [%.2f %.2f %.2f %.2f | %.2f %.2f %.2f %.2f | %.2f %.2f %.2f %.2f]\n",
							dst ? static_cast<unsigned long long>(dst->GetGPUVirtualAddress()) : 0ull, bytes,
							mapped, nz, nanCount, firstNan, nanStride,
							f[0], f[1], f[2], f[3], f[4], f[5], f[6], f[7], f[8], f[9], f[10], f[11]);
						D3D12LogLine(b); g_copyLog++;
					}
				}
				...
				ID3D12Resource* dst, UINT dstSub, ID3D12Resource* src, UINT srcSub, DXGI_FORMAT fmt)
			{
				...
				if (s_rs < 12)
				{
					...
					char b[224];
					_snprintf_s(b, _TRUNCATE, "[resolve] src=%p %llux%u s=%u -> dst=%p %llux%u fmt=%d ra=0x%llX list=%p\n",
			...
			static void STDMETHODCALLTYPE HookedCopyResource(ID3D12GraphicsCommandList* self, ID3D12Resource* dst, ID3D12Resource* src)
			{
				...
				if (fromStfCR || StfRenderActive())
				{
					...
					{
						char b[192];
						_snprintf_s(b, _TRUNCATE, "[copy] Resource dst=%p src=%p dstVA=0x%llX list=%p\n",
			...
			static void STDMETHODCALLTYPE HookedRSSetViewports(ID3D12GraphicsCommandList* self, UINT n, const D3D12_VIEWPORT* vp)
			{
				...
				{
					static int s = 0;
					if (s < 24) { char b[176]; _snprintf_s(b, _TRUNCATE,
				...
				const D3D12_CPU_DESCRIPTOR_HANDLE* rtv, BOOL single, const D3D12_CPU_DESCRIPTOR_HANDLE* dsv)
			{
				...
				{
					static int s = 0;
					if (s < 24) { char b[192]; _snprintf_s(b, _TRUNCATE,
			...
			void SeedCommandContextPool()
			{
				...
				char buf[80];
				_snprintf_s(buf, _TRUNCATE, "[StF cdevice] seeded %u command contexts at +0x28\n", built);
				OutputDebugStringA(buf);
			}
		} // namespace
		...
		void* BuildRenderCommandContext()
		{
			...
			{
				OutputDebugStringA("[StF cdevice] BuildRenderCommandContext failed\n");
				return nullptr;
			}
		...
		void* BuildHostCdevice(ID3D12Device* device, ID3D12CommandQueue* queue, void* (*cdeviceCtor)(void*))
		{
			...
			*reinterpret_cast<uint8_t*>(s_cdevice + 0x8c) = 1;
			...
			// layout-matched one with vendored D3D12MA v2.0.1 (the version the DLL embeds).
			{
				...
				{
					char buf[96];
					_snprintf_s(buf, _TRUNCATE, "[StF cdevice] D3D12MA::CreateAllocator failed hr=0x%08X\n", hr);
					OutputDebugStringA(buf);
				}
			}
...
// each RT StF renders to and pick the one holding the frame. g_rtSeen lives in LJ::StF's anon namespace.
ID3D12Resource* GetStfRenderTarget(int index)
{
	if (index < 0 || index >= LJ::StF::g_rtSeenCount) return nullptr;
	return LJ::StF::g_rtSeen[index];
}
int GetStfRenderTargetCount()
{
	return LJ::StF::g_rtSeenCount;
}
...
{
	if (index < 0 || index >= LJ::StF::g_rtSeenCount) return 0;
	return static_cast<int>(LJ::StF::g_rtLastState[index]);
}
...
{
	return LJ::StF::g_lastStfResolveDst;
}
...
void ExecuteStfRenderListsNow() { LJ::StF::ExecuteStfRenderLists(); }
...
void SubmitStfFrameListNow() { LJ::StF::SubmitStfFrameList(); }
...
void AdvanceFrameStampNow() { LJ::StF::AdvanceFrameStamp(); }
...
bool StfExecDisabledNow() { return LJ::StF::StfExecDisabled(); }
...
void SetStfRenderActiveNow(bool active) { LJ::StF::SetStfRenderActive(active); }

// RenderWindow registers the swapchain backbuffers so the ResourceBarrier hook dumps their transitions.
void RegisterWatchResourceNow(ID3D12Resource* r) { LJ::StF::RegisterWatchResource(r); }
