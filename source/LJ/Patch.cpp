...
namespace LJ
{
	namespace StF
	{
			...
			std::vector<uint64_t>& bitmapOut, const char* label)
		{
			...
			{
				char buf[96];
				_snprintf_s(buf, _TRUNCATE, "[StF gs] CreateDescriptorHeap(%s) failed\n", label);
			...
			*reinterpret_cast<uint64_t*>(a + 0x38) = reinterpret_cast<uint64_t>(bitmapOut.data());
			...
			_snprintf_s(buf, _TRUNCATE,
				"[StF gs] descblock %s @ context+0x%zX: inc=%u cap=%u cpuBase=0x%llX gpuBase=0x%llX\n",
				label, blockOffset, inc, capacity,
		...
		static void InitSrvUavDescriptorHeap(gs::context_t* context, const RenderWindow& window)
		{
			...
			// lighter cap is fine (live peak cursor ~377).
			{
				...
				if (dllHeap != nullptr)
				{
					...
					char buf[128];
					_snprintf_s(buf, _TRUNCATE, "[StF gs] gs+0x7550 rings -> DLL heap %p (num=%u) gpu=0x%llX\n",
						(void*)dllHeap, hd.NumDescriptors, (unsigned long long)gpuBase);
					OutputDebugStringA(buf);
				}
				else
				{
					OutputDebugStringA("[StF gs] WARNING: DLL ring CBV/SRV heap not captured yet!\n");
				}
				(void)kCopyRingCap;
				...
				*reinterpret_cast<void**>(dc + 0x12c88) = s_upCacheTable;
				...
				if (ID3D12DescriptorHeap* samplerHeap = GetDllRingSamplerHeap())
				{
					...
					*reinterpret_cast<uint32_t*>(sc + 0xDC) = 0;                 // entry count (skip search)
					OutputDebugStringA("[StF gs] sampler ring @ device_context+0x11ba0 wired\n");
				}
			}
		...
		static void prj_trap(const char* format, ...)
		{
			...
			if (n < 0)
			{
				...
				OutputDebugStringA("[StF prj_trap] ");
				if (format) OutputDebugStringA(format);
				OutputDebugStringA("\n");
			}
			else
			{
